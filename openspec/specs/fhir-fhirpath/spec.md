...
Évaluateur FHIRPath 2.0 complet en Rust, utilisé pour valider les invariants `constraint.expression` des StructureDefinitions FHIR R5.
...
### Requirement: Grammaire PEG FHIRPath 2.0

Le système SHALL implémenter une grammaire PEG via `pest` couvrant l'intégralité de la syntaxe FHIRPath 2.0.

#### Scenario: Littéraux pris en charge

- **WHEN** l'expression contient un littéral (booléen, entier, décimal, chaîne entre guillemets simples, date `@YYYY-MM-DD`, dateTime `@YYYY-MM-DDThh:mm:ss`, heure `@Thh:mm:ss`, quantité `1.5 'mg'`)
...
Le système SHALL exposer un enum `Expr` couvrant toutes les constructions FHIRPath 2.0.

#### Scenario: Variantes de l'AST

- **WHEN** un AST est construit
- **THEN** `Expr` SHALL couvrir : littéraux (`Null`, `Bool`, `Integer`, `Decimal`, `String`, `Temporal`, `Quantity`), navigation (`DollarThis`, `DollarIndex`, `Ident`, `Dot`, `Index`), appels (`FuncCall`, `Method`), arithmétique (`Add`, `Sub`, `Mul`, `Div`, `DivInt`, `Mod`, `Concat`, `Neg`), comparaisons (`Eq`, `Neq`, `Lt`, `Lte`, `Gt`, `Gte`, `Equiv`, `NotEquiv`), booléens (`And`, `Or`, `Xor`, `Implies`), types (`Is`, `As`), ensembles (`Union`, `In`, `Contains`)
...
- **THEN** `Value` SHALL avoir les variantes : `Null`, `Bool(bool)`, `Integer(i64)`, `Decimal(f64)`, `String(Arc<str>)`, `Date(Arc<str>)`, `DateTime(Arc<str>)`, `Time(Arc<str>)`, `Quantity(f64, Arc<str>)`, `Object(Arc<IndexMap<Arc<str>, Collection>>)`
...
Le système SHALL implémenter `toString()`, `length()`, `startsWith(s)`, `endsWith(s)`, `contains(s)`, `upper()`, `lower()`, `trim()`, `substring(start)`, `substring(start, length)`, `replace(pattern, substitution)`, `matches(regex)`, `replaceMatches(regex, substitution)`, `indexOf(s)`, `split(separator)`, `join(separator)`, `encode(format)`, `decode(format)`.
...
Le système SHALL implémenter `toInteger()`, `toDecimal()`, `toBoolean()`, `convertsToInteger()`, `convertsToDecimal()`, `convertsToBoolean()`, `toDate()`, `toDateTime()`, `toTime()`, `toQuantity()`, `convertsToDate()`, `convertsToDateTime()`, `convertsToTime()`, `convertsToQuantity()`, `toString()`.
...
Le système SHALL implémenter `distinct()`, `isDistinct()`, `subsetOf(other)`, `supersetOf(other)`, `intersect(other)`, `exclude(other)`, ainsi que l'opérateur `|` (union).
...
Le système SHALL implémenter `aggregate(aggregator, init)`, `sum()`, `min()`, `max()`.
